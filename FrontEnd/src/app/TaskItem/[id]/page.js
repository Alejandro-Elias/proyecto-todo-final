...
export default function Page({}) {
  ...
  const [completedStorage, setCompletedStorage] = useState(false)
  ...
  useEffect(() => {
    ...
    if (storeTaskData) {
      ...
      setId(taskData[0].id);
      setCompletedStorage(taskData[0].completed)
    }
  }, []);
      ...
      >
        <div className='bg-[url("/bg-mobile-light.jpg")] dark:bg-[url("/bg-mobile-dark.jpg")] xl:bg-[url("/bg-desktop-light.jpg")] xl:dark:bg-[url("/bg-desktop-dark.jpg")] bg-no-repeat bg-cover h-[200px] px-5 flex flex-col justify-evenly py-3 md:px-[31%] md:h-[260px] md:pt-16'>
          ...
          >
            {({ isSubmitting }) => (
              <Form className="w-80">
                <div className="w-full bg-white h-12 rounded-sm flex items-center dark:bg-veryDarkDesaturatedBlue md:h-[58px] px-3 mt-[32px]">
                  <Field
                    type="text"
                    name="title"
